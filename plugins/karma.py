...
def pluspts(nick, chan, db, conn):
    ...
    for like in likes:
       output = output + "\x02" + str(like[0]) + "\x02 (" + str(like[1]) + ") "
    return output
...
@hook.command("points", "pts", autohelp=False)
def points(text, chan, db, conn):
    ...
    if karma:
        ...
        for k in karma:
            ...
            score += int(k[0])
        if thing:
             return "\x02{}\x02 has a total score of \x02{}\x02 (+{}/{}) across all channels I know about.".format(thing, score, pos, neg)
        return "\x02{}\x02 has a total score of \x02{}\x02 (+{}/{}) in {}.".format(text, score, pos, neg, chan)
    else:
...
def pointstop(text, chan, db, message, conn, notice):
    ...
    if items:
        ...
        for i in range(0,int(ten)):
            out += "\x02{}\x02 ({}), ".format(sorts[i][0], sorts[i][1])
        out = out[:-2]
        return out
...
def pointsbottom(text, chan, db, message, conn, notice):
    ...
    if text == "global":
        items = db.execute("select thing, score from karma").fetchall()
        out = "The {} worst things in all channels are: "
    else:
        items = db.execute("select thing, score from karma where chan = :chan", {'chan':chan}).fetchall()
        out = "The {} worst things in {} are: "
    if items:
        for item in items:
            thing = item[0]
            score = int(item[1])
